...
long long Tot(int Root,int Parent){
    //cout<<"Tot "<<Root<<" "<<Parent<<" Token "<<Tokens[Root]<<endl;
    ...
    //cout<<"Total Sum "<<Root<<" is "<<TotalSum[Root]<<endl;
...
void Trav(int Root,int Parent){
    ...
    /*cout<<"Trav Root "<<Root<<" maxpath "<<maxpath<<endl;
    for(int i=0;i<TotalSub.size();i++){
        cout<<"Total Sub "<<i<<" "<<TotalSub[i]<<endl;
    }*/
    for(int i=0;i<adjlist[Root].size();i++){
...
int main() {
    ...
    //for(int i=0;i<N;i++){
        //cout<<"Traversing with Root "<<i<<endl;
    Tot(1,-1);
    RootReal=1;
    Trav(1,-1);
        //maxpath=max(maxpath,y);
    //}
    cout<<maxpath<<endl;
    return 0;
}